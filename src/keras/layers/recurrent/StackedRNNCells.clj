(ns keras.layers.recurrent.StackedRNNCells
...
(defn StackedRNNCells 
  "Wrapper allowing a stack of RNN cells to behave as a single cell.
    ...
    "
  [ & {:keys [cells]} ]
   (py/call-attr-kw recurrent "StackedRNNCells" [] {:cells cells }))
...
(defn call 
  ""
  [self  & {:keys [inputs states constants]} ]
    (py/call-attr-kw recurrent "call" [self] {:inputs inputs :states states :constants constants }))
...
(defn output-size 
  ""
  [ self ]
    (py/call-attr recurrent "output_size"  self))
...
(defn state-size 
  ""
  [ self ]
    (py/call-attr recurrent "state_size"  self))
