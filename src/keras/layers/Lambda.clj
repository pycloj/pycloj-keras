...
(defn Lambda 
  ...
  [ & {:keys [function output_shape mask arguments]} ]
   (py/call-attr-kw layers "Lambda" [] {:function function :output_shape output_shape :mask mask :arguments arguments }))
...
(defn call 
  ...
  [self  & {:keys [inputs mask]} ]
    (py/call-attr-kw layers "call" [self] {:inputs inputs :mask mask }))
