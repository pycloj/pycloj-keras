(ns keras.layers.TimeDistributed
...
(defn TimeDistributed 
  "This wrapper applies a layer to every temporal slice of an input.
    ...
    "
  [ & {:keys [layer]} ]
   (py/call-attr-kw layers "TimeDistributed" [] {:layer layer }))

(defn activity-regularizer 
  ""
  [ self ]
    (py/call-attr layers "activity_regularizer"  self))
...
(defn call 
  ""
  [self  & {:keys [inputs training mask]} ]
    (py/call-attr-kw layers "call" [self] {:inputs inputs :training training :mask mask }))
...
(defn trainable 
  ""
  [ self ]
    (py/call-attr layers "trainable"  self))
