(ns keras.layers.advanced-activations.ReLU
...
(defn ReLU 
  "Rectified Linear Unit activation function.
    ...
    # Arguments
        ...
        threshold: float. Threshold value for thresholded activation.
    "
  [ & {:keys [max_value negative_slope threshold]
       :or {negative_slope 0.0 threshold 0.0}} ]
  
   (py/call-attr-kw advanced-activations "ReLU" [] {:max_value max_value :negative_slope negative_slope :threshold threshold }))
...
(defn call 
  ""
  [self  & {:keys [inputs]} ]
    (py/call-attr-kw advanced-activations "call" [self] {:inputs inputs }))
