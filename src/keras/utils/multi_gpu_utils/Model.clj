(ns keras.utils.multi-gpu-utils.Model
...
(defn Model 
  "The `Model` class adds training & evaluation routines to a `Network`.
    "
  [  ]
  (py/call-attr multi-gpu-utils "Model"   ))
...
(defn compile 
  ...
  [self  & {:keys [optimizer loss metrics loss_weights sample_weight_mode weighted_metrics target_tensors]} ]
    (py/call-attr-kw multi-gpu-utils "compile" [self] {:optimizer optimizer :loss loss :metrics metrics :loss_weights loss_weights :sample_weight_mode sample_weight_mode :weighted_metrics weighted_metrics :target_tensors target_tensors }))
...
(defn evaluate 
  ...
  [self & {:keys [x y batch_size verbose sample_weight steps]
                       :or {verbose 1}} ]
    (py/call-attr-kw multi-gpu-utils "evaluate" [] {:x x :y y :batch_size batch_size :verbose verbose :sample_weight sample_weight :steps steps }))
...
(defn evaluate-generator 
  ...
  [self & {:keys [generator steps max_queue_size workers use_multiprocessing verbose]
    ...
    (py/call-attr-kw multi-gpu-utils "evaluate_generator" [] {:generator generator :steps steps :max_queue_size max_queue_size :workers workers :use_multiprocessing use_multiprocessing :verbose verbose }))
...
(defn get-layer 
  ...
  [self  & {:keys [name index]} ]
    (py/call-attr-kw multi-gpu-utils "get_layer" [self] {:name name :index index }))
...
(defn input-spec 
  "Gets the model's input specs.
        ...
        "
  [ self ]
    (py/call-attr multi-gpu-utils "input_spec"  self))

(defn layers 
  ""
  [ self ]
    (py/call-attr multi-gpu-utils "layers"  self))
...
(defn load-weights 
  ...
  [self & {:keys [filepath by_name skip_mismatch reshape]
    ...
    (py/call-attr-kw multi-gpu-utils "load_weights" [] {:filepath filepath :by_name by_name :skip_mismatch skip_mismatch :reshape reshape }))
...
(defn predict 
  ...
  [self & {:keys [x batch_size verbose steps]
                       :or {verbose 0}} ]
    (py/call-attr-kw multi-gpu-utils "predict" [] {:x x :batch_size batch_size :verbose verbose :steps steps }))
...
(defn predict-generator 
  ...
  [self & {:keys [generator steps max_queue_size workers use_multiprocessing verbose]
                       :or {max_queue_size 10 workers 1 use_multiprocessing false verbose 0}} ]
    (py/call-attr-kw multi-gpu-utils "predict_generator" [] {:generator generator :steps steps :max_queue_size max_queue_size :workers workers :use_multiprocessing use_multiprocessing :verbose verbose }))
...
(defn predict-on-batch 
  "Returns predictions for a single batch of samples.
        ...
        "
  [self  & {:keys [x]} ]
    (py/call-attr-kw multi-gpu-utils "predict_on_batch" [self] {:x x }))

(defn reset-states 
  ""
  [ self ]
  (py/call-attr multi-gpu-utils "reset_states"  self ))
...
(defn run-internal-graph 
  ...
  [self  & {:keys [inputs masks]} ]
    (py/call-attr-kw multi-gpu-utils "run_internal_graph" [self] {:inputs inputs :masks masks }))
...
(defn save 
  ...
  [self & {:keys [filepath overwrite include_optimizer]
    ...
    (py/call-attr-kw multi-gpu-utils "save" [] {:filepath filepath :overwrite overwrite :include_optimizer include_optimizer }))
...
(defn save-weights 
  ...
  [self & {:keys [filepath overwrite]
                       :or {overwrite true}} ]
    (py/call-attr-kw multi-gpu-utils "save_weights" [] {:filepath filepath :overwrite overwrite }))
...
(defn state-updates 
  "Returns the `updates` from all layers that are stateful.
        ...
        "
  [ self ]
    (py/call-attr multi-gpu-utils "state_updates"  self))

(defn stateful 
  ""
  [ self ]
    (py/call-attr multi-gpu-utils "stateful"  self))
...
(defn summary 
  ...
  [self  & {:keys [line_length positions print_fn]} ]
    (py/call-attr-kw multi-gpu-utils "summary" [self] {:line_length line_length :positions positions :print_fn print_fn }))
...
(defn test-on-batch 
  ...
  [self  & {:keys [x y sample_weight]} ]
    (py/call-attr-kw multi-gpu-utils "test_on_batch" [self] {:x x :y y :sample_weight sample_weight }))
...
(defn to-json 
  "Returns a JSON string containing the network configuration.
        ...
        "
  [ self ]
  (py/call-attr multi-gpu-utils "to_json"  self ))
...
(defn to-yaml 
  "Returns a yaml string containing the network configuration.
        ...
        "
  [ self ]
  (py/call-attr multi-gpu-utils "to_yaml"  self ))
...
(defn train-on-batch 
  ...
  [self  & {:keys [x y sample_weight class_weight]} ]
    (py/call-attr-kw multi-gpu-utils "train_on_batch" [self] {:x x :y y :sample_weight sample_weight :class_weight class_weight }))
...
(defn uses-learning-phase 
  ""
  [ self ]
    (py/call-attr multi-gpu-utils "uses_learning_phase"  self))
